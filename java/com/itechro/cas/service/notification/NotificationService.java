...
@Service
public class NotificationService implements EnvironmentAware {
    ...
    public void notifyFacilityPaperUpdate(FacilityPaperStatusTransitionRQ emailRQ) throws AppsException {

        LOG.info("START : Facility Paper update email notification for assigned user: {} ", emailRQ.getUserName());
        EmailUnSubscribeDTO emailUnSubscribeDTO = null;
        FacilityPaperEmailSender facilityPaperEmailSender = new FacilityPaperEmailSender(emailRQ);
        emailUnSubscribeDTO = emailUnSubscribeJdbcDao.getEmailUnSubscribeDTOByUser(emailRQ.getUserName(), null);
        LOG.info("emailRQ.getToAddresses() {} ", emailRQ.getToAddresses());

        if (emailRQ.getToAddresses().size() == 0) {
            ...
            LOG.info("UpmDetailResponse: {} ", response);
            ...
            if (response.getEmail() != null) {
                ...
                LOG.info("emailAddressFromUpm: {} ", emailAddressFromUpm);

                if (emailUnSubscribeDTO != null) {
                    if (emailUnSubscribeDTO.getStatus() == AppsConstants.Status.ACT) {
                        LOG.info("END : Facility Paper update email notification unsubscribe by assigned user: {} ", emailRQ.getUserName());
                    } else {

                        facilityPaperEmailSender.setCasEmailService(casEmailService);
                        facilityPaperEmailSender.setEnvironment(environment);
                        facilityPaperEmailSender.sendFacilityPaperUpdateEmail();
                        LOG.info("END : Facility Paper update email notification for assigned user: {} ", emailRQ.getUserName());
                    }
                } else {
                    facilityPaperEmailSender.setCasEmailService(casEmailService);
                    facilityPaperEmailSender.setEnvironment(environment);
    ...
    public EmailUnSubscribeDTO updateEmailNotificationGetStatus(EmailUnSubscribeRQ emailUnSubscribeRQ, CredentialsDTO credentialsDTO) throws AppsException {
        LOG.info("Start : Facility Paper update email notification status change by user: {} ", emailUnSubscribeRQ.getUserName());
        ...
        LOG.info("END : Facility Paper update email notification status change by user: {} ", emailUnSubscribeRQ.getUserName());
